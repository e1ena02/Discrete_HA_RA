...
%         end
    ...
    params(end+1) = setup.Params(4, name, quarterly_b_params);
    params(end).phiL = 0;
    params(end).phiH = 1; % [0.1, 0.5, 1, 2, 10]
    params(end).theta = 0.25; %0, .25
    params(end).descr = 'Expense heterogeneity';
    params(end).tex_header = 'Expense Heterogeneity';
    % 
    % % How I think it should be run in for-loop -- not sure if right
    % for p = [0.1, 0.5, 1, 2, 10]
    %     for t = [0, 0.25]
    %         name = sprintf('ExpenseHet, phi = %g, theta = %g', p, t);
    %         params(end+1) = setup.Params(4, name, quarterly_b_params);
    %         params(end).phiL = 0;
    %         params(end).phiH = p;
    %         params(end).theta = t;
    %         params(end).descr = sprintf('High Phi = %g, theta = %g', p, t);
    %     end
    % end


    % name = 'Employment Heterogeneity';
    % params(end + 1) = setup.Params(4, name, quarterly_b_params);
    % params(end).nz = 3; %% 0 means employed, 1 unemployed, 2, long-term unemployed
    % params(end).prob_zswitch = .01;
    % params(end).zdist_forced = ;
