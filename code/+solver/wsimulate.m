function sim_results = simulate_w(p,income,model,grids,heterogeneity)
...
%     
    ...
    %%Simulate Phi
    %% Simulate phi transitions
phirand = rand(p.Nsim,p.Tsim);
phiindsim = zeros(p.Nsim,p.Tsim,'int8');
% Initial distribution
[~,phiindsim(:,1)] = max(phirand(:,1)<=p.phi_trans(1,:)',[],2);

% Transitions over time
for it = 2:p.Tsim
    for i = 1:p.Nsim
        if phiindsim(i,it-1) == 1  % if previously at phiL
            phiindsim(i,it) = (phirand(i,it) > p.phi_trans(1,1)) + 1;  % 1 if stays at phiL, 2 if moves to phiH
        else  % if previously at phiH
            phiindsim(i,it) = (phirand(i,it) < p.phi_trans(2,2)) + 1;  % 1 if moves to phiL, 2 if stays at phiH
        end
    end
end

% Convert indices to actual phi values
phisim = zeros(p.Nsim,p.Tsim);
phisim(phiindsim == 1) = p.phiL;
phisim(phiindsim == 2) = p.phiH;
    ...
    csim1 = zeros(p.Nsim,p.Tsim);
    csim2 = zeros(p.Nsim, p.Tsim);
    ...
    for it = 1:p.Tsim
        ...
        csim1(:,it) = (xsim(:,it) - ssim(:,it) -...
            p.compute_savtax(ssim(:,it))) ./(1 + phisim(:, it));
        csim2(:, it) = csim1(:, it) .* phisim(:, it);
    ...
    simvals.csim1 = csim1(:,end-Tmax+1:end);
    simvals.csm2 = csim2(:, end-Tmax+1:end);
